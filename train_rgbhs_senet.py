...
class AlexNet(nn.Module):  # alexnet with senet
    def __init__(self, num_classes):
        ...
        self.features = nn.Sequential(
            nn.Conv2d(5, 64, kernel_size=11, stride=4, padding=2),  # 64  223 223
...
def train_alexnet():
    ...
    for cnt in range(round(num_epochs / num_each_epoch)):
        ...
        for epoch in range(num_each_epoch):
            ...
            f = open('/home/huyoupeng/flower_classification/Flower-identification-by-ensemble-CNNs/result/{}record_rgbhs_senet.txt'.format(now), 'a', encoding='utf-8', newline='')
            csv_writer = csv.writer(f)
            csv_writer.writerow([str(epoch + 1 + cnt * num_each_epoch), str(corrext_rate)])
            torch.save(model.state_dict(), './checkpoints/rgbhs_alexnet_senet.pth')  # 一个epoch保存一次模型
