...
num_epochs = 100		#要求num_each_epoch的倍数
num_each_epoch = 5
batch_size = 256	#大batch size在显存能允许的情况下收敛速度是比较快的但有时的确会有陷入局部最小的情况
...
class AlexNet(nn.Module):
    def __init__(self, num_classes):
        ...
        self.features = nn.Sequential(
            ...
            nn.MaxPool2d(kernel_size=3, stride=1),					# 64 9 9->64  7  7
            nn.Conv2d(64, 192, kernel_size=3, padding=1),			# 64 7 7->192 7  7

            nn.BatchNorm2d(192, momentum=momentum_num),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=3, stride=1),    			# 192 7 7->192 5  5
            nn.Conv2d(192, 384, kernel_size=3, padding=1),  	# 192 5 5->384 5  5

            nn.BatchNorm2d(384, momentum=momentum_num),
            nn.ReLU(inplace=True),
            nn.Conv2d(384, 256, kernel_size=3, padding=1),  #256 5 5

            nn.BatchNorm2d(256, momentum=momentum_num),
            nn.ReLU(inplace=True),
            nn.Conv2d(256, 256, kernel_size=3, padding=1),  #256 5 5
    ...
    def forward(self, x):
        x = self.features(x)
        x = x.view(x.size(0), 256 * 2 * 2)	      #尺度变换（256,2,2)->(256*2*2,0,0)
        x = self.classifier(x)
        return x
...
for cnt in range(round(num_epochs/num_each_epoch)):
	...
	#------------------- evaluation --------------------------------
	net=torch.load(root + 'ModelBackup/epoch' + str(epoch + 1 + cnt*num_each_epoch) + model_name)
	#print(net)
	net.eval()
