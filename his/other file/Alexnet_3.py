...
model_name = 'flower_model_RGB.pkl'			#模型名称
#train_test = 0 		#0为train 1为test

num_epochs = 900		#要求num_each_epoch的倍数
num_each_epoch = 5
batch_size = 32		#大batch size在显存能允许的情况下收敛速度是比较快的但有时的确会有陷入局部最小的情况
...
############################ main ####################################
train_data=MyDataset(txt=root+'train1_1.txt', transform=img_transforms)
train_loader = DataLoader(dataset=train_data, batch_size=batch_size, shuffle=True)
#用来将自定义的数据读取接口的输出或者PyTorch已有的数据读取接口的输入按照
#	batch size封装成Tensor，后续只需要再包装成Variable即可作为模型的输入

test_data=MyDataset(txt=root+'test1_1.txt', transform=img_transforms)
...
for cnt in range(round(num_epochs/num_each_epoch)):
	...
	model.train()
	torch.save(model, root + 'ModelBackup/acc_' + str(epoch + 1 + cnt*num_each_epoch) \
