...
class MyDataset(Dataset):
    def __init__(self, txt, transform=img_transforms, target_transform=img_transforms, loader=default_loader):
        ...
        for line in fh:
            ...
            line = line.rstrip()
            words = line.split('   ')
            imgs.append((root + 'dwt_segmim/' + words[0], int(words[1])))
        self.imgs = imgs
...
def train_alexnet():
    ...
    for cnt in range(round(num_epochs / num_each_epoch)):
        ...
        for epoch in range(num_each_epoch):
            ...
            f = open('/home/huyoupeng/flower_classification/Flower-identification-by-ensemble-CNNs/result/{}record_dwt_senet.txt'.format(now), 'a', encoding='utf-8', newline='')
            csv_writer = csv.writer(f)
            csv_writer.writerow([str(epoch + 1 + cnt * num_each_epoch), str(corrext_rate)])
            torch.save(model.state_dict(), './checkpoints/dwt_alexnet_senet.pth')  # 一个epoch保存一次模型
