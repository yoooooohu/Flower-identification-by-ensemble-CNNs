...
torch.cuda.set_device(6)  # use the chosen gpu
...
class MyDataset(Dataset):
    def __init__(self, txt, transform=img_transforms, target_transform=img_transforms, loader=default_loader):
        ...
        for line in fh:
            ...
            line = line.rstrip()
            words = line.split('   ')
            imgs.append((root + 'hog_segmim2/' + words[0].split('.')[0] + '.mat', int(words[1])))
            # print(root + 'hog_segmim2/' + words[0].split('.')[0] + '.mat', int(words[1]))
            # print(xxxx)
        self.imgs = imgs
...
class AlexNet(nn.Module):  # alexnet with senet
    def __init__(self, num_classes):
        super(AlexNet, self).__init__()
        self.features = nn.Sequential(
            nn.Conv2d(36, 64, kernel_size=11, stride=4, padding=2),  # 64  223 223
    ...
    def forward(self, x):
        ...
        '''在原来的alex序列中最后一个卷积层与relu函数之间加入了通道注意力机制（senet），以及残差链连接'''
        x = self.conv1(x)
        x = self.bn1(x)
...
def train_alexnet():
    ...
    for cnt in range(round(num_epochs / num_each_epoch)):
        ...
        for epoch in range(num_each_epoch):
            ...
            f = open('/home/huyoupeng/flower_classification/Flower-identification-by-ensemble-CNNs/result/{}record_hog_senet.txt'.format(now), 'a', encoding='utf-8', newline='')
            csv_writer = csv.writer(f)
            csv_writer.writerow([str(epoch + 1 + cnt * num_each_epoch), str(corrext_rate)])
            torch.save(model.state_dict(), './checkpoints/hog_alexnet_senet.pth')  # 一个epoch保存一次模型
